...
import { Trophy, Plus, Camera, X, ChevronDown, ExternalLink, CloudRain } from "lucide-react";
...
function PhotoUploader({
  raceDate, sailor, photos,
}: {
  raceDate: string; sailor: string; photos: RacePhoto[];
}) {
...
export function ResultsView({
  ...
  function handleSave(formData: FormData) {
    ...
    startTransition(async () => {
      await saveResult(raceDate, place, fleetSize, notes, selectedCrew.length > 0 ? selectedCrew : null, resultsUrl);
      setShowForm(false);
      setSelectedCrew([]);
      router.refresh();
    });
  ...
  function handleMarkCancelled(raceDate: string, isCancelled: boolean) {
    startTransition(async () => {
      if (isCancelled) {
        await clearRaceOverride(raceDate);
      } else {
        await setRaceOverride(raceDate, "cancelled", "Weather", sailor);
      }
      router.refresh();
    });
  }

  // All past dates: from results + from availability
    ...
    <div className="space-y-3">
      ...
      {showForm ? (
        <Card>
          <CardContent className="px-4 py-4">
            <form action={handleSave} className="space-y-3">
              ...
              <input name="raceDate" type="date" required className="w-full rounded-md border bg-transparent px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-ring" />
              <div className="grid grid-cols-2 gap-2">
                <input name="place" type="number" min="1" placeholder="Place (e.g. 3)" className="rounded-md border bg-transparent px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-ring" />
                <input name="fleetSize" type="number" min="1" placeholder="Fleet size" className="rounded-md border bg-transparent px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-ring" />
              </div>
              <input name="notes" type="text" placeholder="Notes (optional)" className="w-full rounded-md border bg-transparent px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-ring" />
              <input name="resultsUrl" type="url" placeholder="Clubspot results URL (optional)" className="w-full rounded-md border bg-transparent px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-ring" />
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {crew.map((name) => (
                    <Button key={name} type="button" variant="outline" size="sm"
                      className={selectedCrew.includes(name) ? "bg-emerald-600 text-white border-emerald-600 hover:bg-emerald-700" : "text-muted-foreground"}
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="ghost" type="button" onClick={() => { setShowForm(false); setSelectedCrew([]); }}>Cancel</Button>
              </div>
            </form>
          </CardContent>
        </Card>
        ...
        <Button variant="outline" className="w-full" onClick={() => setShowForm(true)}>
          <Plus className="h-4 w-4 mr-1" /> Log Race Result
        </Button>
      )}
      ...
      {sortedDates.map((date) => {
        ...
        const inNames = loggedCrew.length > 0
          ? loggedCrew
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {isCancelled ? `Cancelled · ${override?.reason ?? ""}` : result?.notes ?? ""}
                </p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {isCancelled && <CloudRain className="h-3.5 w-3.5 text-muted-foreground" />}
            ...
            {isExpanded && (
              <CardContent className="px-4 pb-4 pt-0 border-t space-y-3">
                ...
                <Button
                  ...
                  size="sm"
                  disabled={isPending}
                  className={isCancelled ? "text-muted-foreground" : "text-muted-foreground"}
                  onClick={() => handleMarkCancelled(date, isCancelled)}
                >
                  <CloudRain className="h-3.5 w-3.5 mr-1.5" />
                  {isCancelled ? "Unmark cancelled" : "Mark as cancelled (weather)"}
                </Button>
              </CardContent>
            )}
          </Card>
